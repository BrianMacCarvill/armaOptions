...
#' strike_price <- 110
#' CallOptionsOverTime(stock_data, future_times, strike_price)
#' }
#'
#' @export
CallOptionsOverTime <- function(stock_data, future_times, strike_price, max.p = 5, max.q = 5) {
  # Validate inputs
  ...
  for (i in seq_along(future_times)) {
    ...
    option_value_result <- europeanCallOptionValue(stock_data, future_time, strike_price, max.p, max.q)
